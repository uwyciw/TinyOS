...
#define __OS_TASK_INSERT(__INIT__, __TASK__)                                                       \
    ((OS_TCB){.Init = (void *)(__INIT__), .Task = (void *)(__TASK__)})
 ...
 * @retval
 */
#define __OS_EVENT_ALLOC(__NAME__) OS_EVENT __NAME__ = {.Id = -1, .Timeout = 0}
 ...
 * @retval
 */
typedef void (*OSTaskFunction)(OS_TCB * tcb);

/**
 * @brief 弱类型函数。临界区保护函数。
 * @note 当需要临界区保护时，用户可重定义。
 * @param
 * @retval
 */
 ...
 */
bool OSEventAssert(OS_EVENT * event);
